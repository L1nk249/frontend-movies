...
function Home() {
  ...
  useEffect(()=>{
    fetch('https://vercelmymoviesbackend-levy-cedrics-projects.vercel.app')
...
if (data.overview.length >250){
  over=data.overview.slice(0,250)+"..."
}else{
  over= data.overview
}

    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={over}  poster= {`https://image.tmdb.org/t/p/w500/${data.poster_path}`} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
